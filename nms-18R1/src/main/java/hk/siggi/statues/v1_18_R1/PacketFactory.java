package hk.siggi.statues.v1_18_R1;
...
public class PacketFactory extends hk.siggi.statues.PacketFactory<DataWatcher> {
	...
	public PacketPlayOutPlayerInfo addPlayer(String displayName, GameProfile profile,
		...
		try {
			...
			list.add(new PacketPlayOutPlayerInfo.PlayerInfoData(profile, ping, EnumGamemode.a(gamemode), new ChatComponentText(displayName)));
			return packet;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public PacketPlayOutPlayerInfo removePlayer(String displayName, GameProfile profile) {
		try {
			...
			List list = (List) getField(packet, "b");
			list.add(new PacketPlayOutPlayerInfo.PlayerInfoData(profile, 0, EnumGamemode.a(0), new ChatComponentText(displayName)));
			return packet;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
