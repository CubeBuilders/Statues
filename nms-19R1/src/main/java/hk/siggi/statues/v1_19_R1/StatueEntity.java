...
public class StatueEntity extends EntityHuman implements hk.siggi.statues.StatueEntity<Packet> {
	...
	public StatueEntity(GameProfile profile, Statue statue, NMSUtil nmsUtil, PacketFactory packetFactory) {
		super(((CraftWorld) Bukkit.getServer().getWorlds().get(0)).getHandle().getMinecraftWorld(), new BlockPosition(0.0, 0.0, 0.0), 0.0f, profile, null);
		this.statue = statue;
	...
	protected void a_() {
		...
		super.a_();
		//b(1, false, 16);
		ai().b(getClientSettingsDataWatcher(), (byte) 127);
		ai().b(getMainHandDataWatcher(), (byte) 0);
	}
	...
	public PacketPlayOutEntityMetadata metadata() {
		ai().b(getClientSettingsDataWatcher(), (byte) 127);
		ai().b(getMainHandDataWatcher(), (byte) 0);
		return packetFactory.metadata(statue.entityID, ai());
	}
