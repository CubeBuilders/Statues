package hk.siggi.statues.v1_9_R2;
...
public class PacketFactory extends hk.siggi.statues.PacketFactory<DataWatcher> {
	...
	public PacketPlayOutPlayerInfo addPlayer(String displayName, GameProfile profile,
		...
		try {
			...
			list.add(newPlayerInfoData(packet, profile, ping, WorldSettings.EnumGamemode.getById(gamemode), new ChatComponentText(displayName)));
			return packet;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public PacketPlayOutPlayerInfo removePlayer(String displayName, GameProfile profile) {
		try {
			...
			List list = (List) getField(packet, "b");
			list.add(newPlayerInfoData(packet, profile, 0, WorldSettings.EnumGamemode.getById(0), new ChatComponentText(displayName)));
			return packet;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	private static Class getPIDClass() {
		if (pidclass == null) {
			try {
				pidclass = Class.forName("net.minecraft.server.v1_9_R2.PacketPlayOutPlayerInfo.PlayerInfoData");
			} catch (Exception e) {
				try {
					pidclass = Class.forName("net.minecraft.server.v1_9_R2.PacketPlayOutPlayerInfo$PlayerInfoData");
				} catch (Exception e2) {
	...
	public PacketPlayOutNamedEntitySpawn spawn(int id, GameProfile profile,
		...
		net.minecraft.server.v1_9_R2.ItemStack nmsItemStack = CraftItemStack.asNMSCopy((CraftItemStack) itemInHand);
		try {
