package hk.siggi.statues.v1_9_R1;
...
public class PacketFactory extends hk.siggi.statues.PacketFactory<DataWatcher> {
	...
	private static Class getPIDClass() {
		if (pidclass == null) {
			try {
				pidclass = Class.forName("net.minecraft.server.v1_9_R1.PacketPlayOutPlayerInfo.PlayerInfoData");
			} catch (Exception e) {
				try {
					pidclass = Class.forName("net.minecraft.server.v1_9_R1.PacketPlayOutPlayerInfo$PlayerInfoData");
				} catch (Exception e2) {
	...
	public PacketPlayOutNamedEntitySpawn spawn(int id, GameProfile profile,
		...
		net.minecraft.server.v1_9_R1.ItemStack nmsItemStack = CraftItemStack.asNMSCopy((CraftItemStack) itemInHand);
		try {
