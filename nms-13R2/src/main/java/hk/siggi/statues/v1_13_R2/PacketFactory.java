package hk.siggi.statues.v1_13_R2;
...
public class PacketFactory extends hk.siggi.statues.PacketFactory<DataWatcher> {
	...
	private static Class getPIDClass() {
		if (pidclass == null) {
			try {
				pidclass = Class.forName("net.minecraft.server.v1_13_R2.PacketPlayOutPlayerInfo.PlayerInfoData");
			} catch (Exception e) {
				try {
					pidclass = Class.forName("net.minecraft.server.v1_13_R2.PacketPlayOutPlayerInfo$PlayerInfoData");
				} catch (Exception e2) {
	...
	public PacketPlayOutNamedEntitySpawn spawn(int id, GameProfile profile,
		...
		net.minecraft.server.v1_13_R2.ItemStack nmsItemStack = CraftItemStack.asNMSCopy((CraftItemStack) itemInHand);
		try {
	...
	public PacketPlayOutEntity.PacketPlayOutRelEntityMove move(int entityID, int x, int y, int z, boolean onGround) {
		try {
			...
			setField(PacketPlayOutEntity.class, packet, "b", (short) x);
			setField(PacketPlayOutEntity.class, packet, "c", (short) y);
			setField(PacketPlayOutEntity.class, packet, "d", (short) z);
